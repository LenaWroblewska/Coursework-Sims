...
std::vector<Vector2> GameMap::FindSpawnSpace() {
	std::vector<Vector2> spawnSpace;
	for (int y = 0; y < mapHeight; ++y) {
		for (int x = 0; x < mapWidth; ++x) {
			int tileIndex = (y * mapWidth) + x;
			// If tile is air and has grass below
			if (mapData[tileIndex] == '0' && mapData[tileIndex - 30] == 'G') {
				spawnSpace.emplace_back((Vector2(x,y) * 16) + Vector2(0,8));
			}
		}
	}
	return spawnSpace;
